              ...
              {items.map((item, index) => (
                <tr key={item.id} className="border-b border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors group">
                  ...
                  <td className="p-1.5 text-center no-print">
                    <button onClick={() => removeItem(item.id)} className="text-slate-400 hover:text-red-500 dark:hover:text-red-400 p-0.5 rounded opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all hover:bg-red-50 dark:hover:bg-red-900/20">
                  ...
                  {discountAmount > 0 && (
                    <tr className="bg-green-50 dark:bg-green-900/10">
                      <td colSpan={5} className="p-2 text-right text-xs pr-4 text-green-700 dark:text-green-400">
                        خصم {discountType === "percent" ? `(${discountValue}%)` : ""}
                      </td>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <div className="space-y-1">
              <label className="text-xs text-muted-foreground font-medium">
                قيمة الخصم {discountType === "percent" ? "(%)" : "(د)"}
              </label>
              <input
                type="number"
                min="0"
        ...
        <div className="space-y-2">
          {details.notes.trim() && (
            <div className="space-y-1">
              ...
              <div className="p-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded text-slate-700 dark:text-slate-300 text-xs whitespace-pre-wrap break-words line-clamp-2">{details.notes}</div>
            </div>
          )}
