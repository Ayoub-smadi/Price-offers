            ...
            <div className="overflow-y-auto flex-1 p-3 space-y-2">
              {filteredCatalog.length === 0 ? (
                ...
                filteredCatalog.map(p => (
                  <button key={p.id} onClick={() => addFromCatalog(p)} className="w-full text-right rounded-xl border border-border hover:border-primary hover:bg-primary/5 transition-all flex overflow-hidden group">
                    {p.imageUrl ? (
                      <img
                        src={p.imageUrl}
                        alt={p.name}
